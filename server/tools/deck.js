...
module.exports.updateDeck = async function updateDeck(params) {
    ...
    let toUpdate;
    if (params.title) {
        toUpdate = {title: params.title};
    } else if (params.author) {
        toUpdate = {author: params.author};
    } else if (params.cardsCount) {
        toUpdate = {cardsCount: params.cardsCount};
    } else {
        throw new InternalError('You can update only title, author, createdAt');
    }

    return await Deck.findOneAndUpdate({shortId: params.deckId}, toUpdate, {new: true}).exec().then(function(data) {
        if (data) {
            return data;
        } else {
            throw new InternalError('Deck not found');
